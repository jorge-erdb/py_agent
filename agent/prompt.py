...
The prompt is assembled once per process (see api.main.SYSTEM_PROMPT) and the same string is handed to every Agent the instance creates. Nothing here is per-session or per-turn: anything that varies between sessions belongs in the conversation, not in history[0].

Sections are built independently and joined with blank lines, so an empty section simply drops out rather than leaving a stray heading behind.
"""
...
# Resolved here rather than in api.main so that the prompt and everything that configures it live together — api.main imports these, and `python -m agent.prompt` gets the same values without a circular import.
...
# Free-form operator text appended to the end of the system prompt, after the built-in sections, so it can override them.
...
You have `read_file` tool, for reading files.

- Takes a file path and returns the full contents.
...
You have `write_file` tool, for writing files.

- Takes a file path and content string. Creates parent directories automatically if needed.
- You are responsible for ensuring the content is valid text or code.
...
You have `run_shell_command` tool. It executes any command available in the environment directly.

- Output is truncated at {MAX_OUTPUT_CHARS} characters.
- A command that runs longer than {COMMAND_TIMEOUT_SECONDS} seconds is killed and returns no output at all. Scope expensive searches to a directory rather than starting from `/`.

Any command available in the environment can be executed. If a task genuinely needs something unusual, say so and let the user run it."""
...
- Prefer reading the system over guessing about it. One targeted command beats a paragraph of speculation.
- Report what the command actually returned. If it failed or returned nothing, say that rather than describing what you expected.
...
def build_system_prompt(
    ...
    *,
    ...
    Args:
        ...
        include_shell_policy: Emit the run_shell_command rules. Set False when the agent is created without the shell tool registered.
